...
- **프롬프트 관리** — `src/lib/prompts/`에 목적별 템플릿 분리 (진단, 소크라테스, 파인만, 테스트, 약점 분류 등 9개)
...
│   ├── data/                   # 파일 기반 데이터 계층 (7개 매니저)
│   ├── prompts/                # AI 프롬프트 템플릿 (9개)
│   └── weakness/               # 약점 자동 분류 유틸
├── stores/                     # zustand (topic, session, test, weakness, settings)
└── types/                      # 공유 타입 (topic, session, test, weakness, api)
data/                           # 학습 데이터 (gitignore)
└── topics/{id}/                # meta, roadmap, weaknesses, sessions, tests, growth, qna
```
